...
%==============================================================================
    ...
    \\ \tab
    In terms of how this was compiled, open a Cygwin terminal in the appropriate
    directory with all the source files and run make. Then make clean, and
    ./client or ./server. Note the server also needs a file called programs with
    a .makefile inside of it, as attached, to function.
    \\ \tab Example directories were also inserted so the marker has an easier
    time testing. In the client folder there is sendMe1.txt and sendMe2.txt and
    hello.txt for sending. On the server side, prognames "hello" you can run for
    a simple hello world that includes multiple files. Additionally, the "lots"
    folder just has a large number of empty text files to test the list command
    on. Note that the user should make sure to not delete the .makefile inside
    the server programs since that's used to compile programs in the prognames.
\end{document}